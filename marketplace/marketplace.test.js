...
test("marketplace archive has static Bitrix24 entry files", () => {
  ...
  assert.match(settingsHtml, /settings\.js\?v=layout-20260826-2/);
  assert.match(settingsHtml, /style\.css\?v=layout-20260826-2/);
  assert.match(indexHtml, /app\.js\?v=layout-20260826-2/);
  assert.match(indexHtml, /style\.css\?v=layout-20260826-2/);
  assert.match(workerHtml, /worker\.js\?v=layout-20260826-2/);
  assert.match(indexHtml, /Deal Invoice Summary v\.36 Marketplace B24/);
...
test("marketplace app uses Bitrix24 REST directly without VibeCode backend", () => {
  ...
  assert.match(appJs, /layout-20260826-2/);
  assert.match(appJs, /Deal Invoice Summary v\.36 Marketplace B24/);
...
test("marketplace background worker watches open deal pages without backend", () => {
  assert.match(workerJs, /layout-20260826-2/);
  assert.match(workerJs, /Deal Invoice Summary v\.36 Marketplace B24/);
  ...
  assert.match(workerJs, /background-kanban-return-recalculate/);
  ...
  assert.match(workerJs, /function isDealKanbanUri/);
  assert.match(workerJs, /function rememberLastSeenDeal/);
  assert.match(workerJs, /dealInvoiceSummaryLastSeenDeal/);
...
test("marketplace automation panel presents mass recalculation and statistics", () => {
  ...
  assert.match(settingsHtml, /id="includeInvoiceWindowDeals"/);
  assert.match(settingsHtml, /сделки прошлых периодов/);
  ...
  assert.match(settingsJs, /includeInvoiceWindowDeals/);
  ...
  assert.match(appJs, /includeInvoiceWindowDeals:\s*true/);
  ...
  assert.match(appJs, /const secondsPerDeal = 10/);
  ...
  assert.match(appJs, /crm\.deal\.list/);
  assert.match(appJs, /currentSettings\.includeInvoiceWindowDeals \? getRecentInvoiceDealIds\(days\) : emptyRecentInvoiceWindow\(days\)/);
  assert.match(appJs, /function emptyRecentInvoiceWindow/);
  assert.match(appJs, /function filterDealIdsByCategory/);
  assert.match(appJs, /filter:\s*\{ ID: ids, CATEGORY_ID: selectedCategory \}/);
  assert.match(appJs, /recent\.invoiceDealIds = filteredInvoiceWindow\.dealIds/);
  assert.match(appJs, /recent\.dealIds = \[\.\.\.new Set\(\[\.\.\.recent\.dealIds, \.\.\.filteredInvoiceWindow\.dealIds\]\)\]/);
  assert.match(appJs, /source\.results\.filter\(\(item\) => !item\.skippedCategory\)\.map/);
  assert.match(appJs, /filter:\s*\{ \.\.\.categoryFilter, \[`>=\$\{fieldName\}`\]: since \}/);
  assert.match(appJs, /function downloadWindowReport/);
  assert.match(appJs, /Тип воронки/);
  assert.match(appJs, /Стадия сделки/);
  assert.match(appJs, /function dealCategoryName/);
  assert.match(appJs, /dealCategoryMap/);
  assert.match(appJs, /function loadDealStagesForDeals/);
  assert.match(appJs, /function dealStageName/);
  ...
  assert.match(appJs, /function reloadAppFrame/);
  assert.match(appJs, /document\.querySelector\("#refresh"\)\.addEventListener\("click", reloadAppFrame\)/);
  ...
  assert.match(appJs, /function openDealListForStatsSlice/);
  assert.match(appJs, /BX24\.openPath\(path\)/);
  assert.match(appJs, /windowStatsChart\.style\.cursor = slice\?\.dealIds\?\.length \? "pointer" : "default"/);
  ...
  assert.match(appJs, /function openSupportSettingsChat/);
  assert.match(appJs, /window\.location\.href = marketplaceFileUrl\(settingsPageFileName, \{ openChat: "1" \}\)/);
  assert.match(appJs, /noticeAction\.addEventListener\("click", openSupportSettingsChat\)/);
  ...
  assert.match(styleCss, /\.setup-grid\s*{[\s\S]*grid-template-columns:\s*minmax\(0,\s*1\.1fr\) minmax\(0,\s*\.9fr\)/);
  assert.match(styleCss, /\.setup-grid\s*{[\s\S]*align-items:\s*start/);
  assert.match(styleCss, /@media \(max-width:\s*1120px\)[\s\S]*\.setup-grid\s*{[\s\S]*grid-template-columns:\s*1fr/);
  ...
  assert.match(styleCss, /\.automation-title-row\s*{[\s\S]*grid-template-columns:\s*minmax\(0,\s*1fr\) minmax\(88px,\s*max-content\) 38px/);
  assert.match(styleCss, /\.automation-title-row h2\s*{[\s\S]*min-width:\s*0/);
  assert.match(styleCss, /\.automation-panel button\s*{[\s\S]*max-width:\s*100%/);
  assert.match(styleCss, /\.automation-category/);
  assert.match(styleCss, /\.setting-check/);
  ...
  assert.match(styleCss, /\.modal-backdrop\s*{[\s\S]*padding:\s*clamp\(10px,\s*2vh,\s*20px\)/);
