...
В файле/разделе "Файлы Bitrix24 Marketplace приложения" вместе с составом zip фиксировать минимальные права текущего приложения. Для `Deal Invoice Summary` писать `Пользователи (минимальный) / user_brief`, потому что `user.get` используется только для отображения ФИО ответственных. `Пользователи (базовый) / user_basic`, полный `Пользователи / user` и `Пользовательские поля пользователя / user.userfield` не указывать без реальных вызовов контактных данных, изменения пользователей или `user.userfield.*`.
...
Для `Deal Invoice Summary` пользовательский скоуп должен быть минимальным: `Пользователи (минимальный) / user_brief`. Он нужен только для `user.get`, чтобы показать ФИО ответственных по счетам. `Пользователи (базовый) / user_basic`, полный `Пользователи / user` и `Пользовательские поля пользователя / user.userfield` не нужны, потому что приложение не читает контактные данные, не вызывает `user.add`, `user.update`, `user.userfield.*` и не меняет профили пользователей.

Если приложение использует `app.option.get`, `app.option.set` или `user.admin`, но в Marketplace UI нет отдельного выбираемого права `Базовый / basic`, сначала проверить реальный сценарий установки и работы с минимальными правами: установка, запуск от администратора и запуск от сотрудника без административных прав. Если приложение корректно работает, `basic` не указывать как отдельное право для модерации. В документации написать, что методы используются, но отдельный selectable scope `basic` в заявке Marketplace не выбирается.
