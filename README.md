...
- `POST /api/recalculate/recent` — пересчёт сделок со счетами за последние `AUTO_RECALC_RECENT_HOURS`, по умолчанию 7 часов.
...
- каждый проход пересчитывает сделки из watchlist и сделки, у которых есть счета, созданные за последние `AUTO_RECALC_RECENT_HOURS`, по умолчанию 7 часов;
...
MVP v1 готов для тестовой эксплуатации на портале `vibecode01.bitrix24.ru`. Последний деплой: `v22` на сервер `c576798a-ec5f-4491-b6a2-b28e74fc445f`.
